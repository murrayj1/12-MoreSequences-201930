...
def run_test_pizza():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  30 slices, thin (thickness=3) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 3
    pizza(window, circle, 100, 'black', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # -------------------------------------------------------------------------
    list_points = generate_points_on_circle(circle, number_of_slices)
    circle.attach_to(window)
    for k in range(len(list_points)):
        new_line = rg.Line(list_points[k], circle.center)
...
def run_test_polygon():
    ...
    # Test 3:
    title = '4 Segments, Black Lines, on Lime Green Circle'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 80)
    circle.fill_color = 'green1'
    circle.outline_thickness = 3
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(1, len(list_points)):
        new_line = rg.Line(list_points[k-1], list_points[k])
        new_line.color = color
        new_line.thickness = thickness
        new_line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 4:  51 red lines on black circle,'
             + ' hops = 25.')
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 51, 25, 'red', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    ###########################################################################
    # -------------------------------------------------------------------------
    list_points = generate_points_on_circle(circle, number_of_lines)
    list_points = list_points + [list_points[0]]
    circle.attach_to(window)
    for k in range(1, len(list_points)):
        if (k + hops_to_next_point - 1) < (len(list_points) - 1):
            new_line = rg.Line(list_points[k-1], list_points[k + hops_to_next_point - 1])
            new_line.color = color
            new_line.thickness = thickness
            new_line.attach_to(window)
        else:
            m = (k + hops_to_next_point - 1) % number_of_lines
            new_line = rg.Line(list_points[k - 1], list_points[m])
            new_line.color = color
            new_line.thickness = thickness
            new_line.attach_to(window)
    window.render()
